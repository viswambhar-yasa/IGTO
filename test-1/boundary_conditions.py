...
class BC_Switcher(object):
    ...
    def number_1(self):
        if self.bc_disp:
            ...
            print('='*width)
            fmt='{:^'+str(width)+'}'
            print(fmt.format(' Cantilever beam with pressure at top'))
        dof=3
        fixed_nodes=np.array([index for index,j in enumerate(self.CONTROL_POINTS) if self.CONTROL_POINTS[index,0]==0])
        fixed_indicies=np.sort(np.concatenate((fixed_nodes*dof,dof*fixed_nodes+1,fixed_nodes*dof+2)))
    ...
    def number_3(self):
        if self.bc_disp:
            ...
            print('='*width)
            fmt='{:^'+str(width)+'}'
            print(fmt.format('  2d case loading at y=height and x=length \n'))
        dof=3
        fixed_nodes=np.array([index for index,j in enumerate(self.CONTROL_POINTS) if self.CONTROL_POINTS[index,0]==0])
        fixed_indicies=np.sort(np.concatenate((fixed_nodes*dof,dof*fixed_nodes+1,fixed_nodes*dof+2)))

        load_nodes=np.array([index for index,j in enumerate(self.CONTROL_POINTS) if (self.CONTROL_POINTS[index,0]==self.length and  self.CONTROL_POINTS[index,1]==self.height or self.CONTROL_POINTS[index,2]==0 )] )
        load_indicies=np.sort((dof*load_nodes+1))
        return fixed_indicies,load_indicies,fixed_nodes,load_nodes
