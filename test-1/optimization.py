...
def optimality_criteria(dfun,dCon,initial_X,constrain,H=None,DH=None,beta=None,oc_disp=False,g=1,tol=0.01,move=0.1,neta=0.5,initial_value=0,final_value=1e09):
    ...
    def prime_dual(L,U,alpha,beta,p0,q0,pc,qc,a,b,c,d,n=nel,m=vel,epsimin=1e-7):
        ...
        def Newton_method(U,L,x,y,z,alpha,beta,p0,q0,pc,qc,epsi,lamda,s,Zeta,eta,neta,nu,residumax,residunorm,variable_vector=variable_vector,minimizer_vector=minimizer_vector):
            ...
            while iteration<max_iteration:
                ...
                oldZeta=Zeta
                olds=s
                new_x,new_y,new_z,new_lamda,new_eta,new_neta,new_nu,new_Zeta,new_s,new_residualmax,new_residunorm=line_search(w,dw,x,y,z,lamda,eta,neta,nu,Zeta,s,dx,dy,dz,dlamda,deta,dneta,dnu,dZeta,ds,residnorm,epsi)
                x=new_x
                y=new_y
                z=new_z
                lamda=new_lamda
                ...
                if residumax<0.9*epsi :#or np.linalg.norm(abs(exit_cond))<1e-4:
                    ...
                    return x,y,z,lamda,eta,neta,nu,Zeta,s
